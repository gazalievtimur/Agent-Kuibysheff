...
<#
.SYNOPSIS
  Оркестратор воркфлоу 1С на Agent Kuibysheff: intake -> analyst -> coder -> implementer.
...
param(
    ...
    # Путь к бинарнику agent_Kuibysheff, либо "cargo" для сборки на лету
    [string] $AgentBin = "",

    # Корень каталогов прогонов (по умолчанию: ProjectRoot/.kuibysheff/runs или workflows/1c-dev/runs)
    [string] $RunsRoot = "",

    # Папка продукта 1С (VS Code workspace). Когда задана — configs/homes под .kuibysheff/
    [string] $ProjectRoot = "",

    # Идентификатор прогона; если пусто — генерируется из IssueKey/TaskFile + timestamp
    [string] $RunId = "",

    # Корень репозитория Agent Kuibysheff; пусто = родитель каталога scripts/
    [string] $RepoRoot = "",

    # Корень copy unit workflows/1c-dev (prompts/adapters/agents/products)
    [string] $WorkflowRoot = "",
...
function Get-KuibysheffRelativeHome {
    param(
        [string]$ProjectRoot,
        [string]$AbsoluteHome
    )
    $kuib = [System.IO.Path]::GetFullPath((Join-Path $ProjectRoot ".kuibysheff"))
    $homeFull = [System.IO.Path]::GetFullPath($AbsoluteHome)
    if (-not $homeFull.StartsWith($kuib, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Agent home must be under .kuibysheff/: $AbsoluteHome"
    }
    $rel = $homeFull.Substring($kuib.Length).TrimStart('\', '/')
    return ($rel -replace '\\', '/')
}
...
function Invoke-AgentRun {
    param(
        ...
        [string]$Bin,
        # Корень продукта (--project-root)
        [string]$ProjectRoot,
        # Id агента (--agent)
        [string]$AgentId,
        # Полный текст промпта этапа
        [string]$Prompt,
        # Абсолютный home; передаётся как relative под .kuibysheff/
        [string]$HomeDir,
        # Доп. файлы для --files
        [string[]]$Files,
    ...
    $homeRel = Get-KuibysheffRelativeHome -ProjectRoot $ProjectRoot -AbsoluteHome $HomeDir
...
if (-not $RepoRoot) {
    # Prefer monorepo root when this package lives under workflows/<name>/.
    ...
    } elseif (Test-Path -LiteralPath (Join-Path $PSScriptRoot "prompts")) {
        # Detached copy unit without a checkout.
        $RepoRoot = ""
    } else {
        $RepoRoot = Resolve-Path $parent
    }
...
foreach ($dotenv in $dotenvCandidates) {
...
# $workflowRoot — корень пакета воркфлоу (copy unit)
if ($WorkflowRoot) {
    ...
    throw "Specify -WorkflowRoot (1C workflow copy unit root)"
}
if (-not (Test-Path -LiteralPath (Join-Path $workflowRoot "prompts") -PathType Container)) {
    throw "WorkflowRoot missing prompts/: $workflowRoot"
}

# $ProjectRootForAgent — передаётся в agent_Kuibysheff --project-root
...
# $productPath — YAML адаптера: .kuibysheff/product.yaml или products/<Product>.yaml
$productPath = $null
if ($script:ProjectRootForAgent) {
    $inProject = Join-Path $script:ProjectRootForAgent ".kuibysheff\product.yaml"
    if (Test-Path -LiteralPath $inProject -PathType Leaf) {
        $productPath = $inProject
    }
}
if (-not $productPath) {
    $productPath = Join-Path $workflowRoot "products\$Product.yaml"
}
if (-not (Test-Path -LiteralPath $productPath -PathType Leaf)) {
    throw "Product config not found: $productPath (scaffold .kuibysheff/product.yaml or copy products/*.yaml.example)"
}
...
if (-not $RunsRoot) {
    if ($script:ProjectRootForAgent) {
        $RunsRoot = Join-Path $script:ProjectRootForAgent ".kuibysheff\runs"
    } else {
...
if (-not $AgentBin) {
    $onPath = Get-Command agent_Kuibysheff -ErrorAction SilentlyContinue
    if ($onPath) {
        $AgentBin = $onPath.Source
    } elseif ($RepoRoot) {
        $release = Join-Path $RepoRoot "target\release\agent_Kuibysheff.exe"
        $debugBin = Join-Path $RepoRoot "target\debug\agent_Kuibysheff.exe"
        ...
        throw "agent_Kuibysheff not found on PATH; pass -AgentBin or run from a monorepo with target/release"
    }
}

# Profiles live under {ProjectRoot}/.kuibysheff/protected/agents/<id>
if (-not $script:ProjectRootForAgent) {
    throw "1c-dev-run requires -ProjectRoot (agent profiles live under .kuibysheff/protected/agents/)"
}

# $agentProfiles — Id + template source for import; Prompt under WorkflowRoot
$agentProfiles = @{
    "1" = @{ Id = "1c-intake"; Template = "agents/1c-intake"; Prompt = "prompts/stage1.intake.md" }
    "2" = @{ Id = "1c-analyst"; Template = "agents/1c-analyst"; Prompt = "prompts/stage2.analysis.md" }
    "3" = @{ Id = "1c-coder"; Template = "agents/1c-coder"; Prompt = "prompts/stage3.coder.md" }
    "4" = @{ Id = "1c-implementer"; Template = "agents/1c-implementer"; Prompt = "prompts/stage4.implement.md" }
}
...
function Ensure-AgentProfile {
    ...
    $profileDir = Join-Path $script:ProjectRootForAgent ".kuibysheff\protected\agents\$AgentId"
    $configPath = Join-Path $profileDir "agent-config.yaml"
    if ((Test-Path -LiteralPath $configPath -PathType Leaf) -and (Test-Path -LiteralPath (Join-Path $profileDir "skills.dsl"))) {
        return
    }
    $template = Resolve-RepoPath $workflowRoot $TemplateRel
    if (-not (Test-Path -LiteralPath $template -PathType Container)) {
        if ($RepoRoot) {
            $template = Resolve-RepoPath $RepoRoot ("test-agents/" + $AgentId)
        }
    }
    if (-not (Test-Path -LiteralPath $template -PathType Container)) {
        throw "Missing agent template for ${AgentId}: $template (run scaffold-project or place profiles under .kuibysheff/protected/agents/)"
    }
    Write-Host "Ensuring protected profile $AgentId from $template"
    if ($AgentBin -eq "cargo") {
        throw "Cannot auto-import agent profile while AgentBin=cargo; pass -AgentBin or scaffold the project first"
    }
    & $AgentBin init $AgentId --project-root $script:ProjectRootForAgent --force 2>$null | Out-Null
    & $AgentBin config --project-root $script:ProjectRootForAgent --agent $AgentId import --from $template --force
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to import agent profile $AgentId"
    }
}
...
function Run-Stage {
    ...
    Ensure-AgentProfile -AgentId $profile.Id -TemplateRel $profile.Template

    if ($ConfigOverride) {
        Write-Warning "-ConfigOverride is ignored; edit the protected profile via ``agent_Kuibysheff config`` / import"
    }

    # $promptTpl — шаблон промпта этапа с плейсхолдерами
    $promptTpl = Get-Content -LiteralPath (Resolve-RepoPath $workflowRoot $profile.Prompt) -Raw -Encoding UTF8
    ...
    if ($AgentBin -eq "cargo") {
        if (-not $RepoRoot) {
            throw "cargo agent mode requires -RepoRoot (monorepo checkout); pass -AgentBin instead"
        }
        ...
        $allFiles = @($promptFile) + @($files | Where-Object { $_ })
        $homeRel = Get-KuibysheffRelativeHome -ProjectRoot $script:ProjectRootForAgent -AbsoluteHome $stageHome
        # $argList — argv для `cargo run --bin agent_Kuibysheff -- run …`
        $argList = @(
            "run", "--bin", "agent_Kuibysheff", "--",
            "run",
            "--project-root", $script:ProjectRootForAgent,
            "--agent", $profile.Id,
            "--home", $homeRel,
            "--prompt", $shortPrompt
        )
        ...
        Write-Host ">> cargo run --bin agent_Kuibysheff -- run ..."
        ...
        $runResult = Invoke-AgentRun -Bin $AgentBin `
            -ProjectRoot $script:ProjectRootForAgent -AgentId $profile.Id `
